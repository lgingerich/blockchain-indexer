...
async fn test_indexing_pipeline() -> Result<()> {
    ...
    let test_cases = vec![
        // Ethereum blocks
        (Chain::Ethereum, "Ethereum", ETH_RPC_URL, ETH_PARAMS.to_vec()),
        // ZKSync blocks
        (Chain::ZKsync, "ZKsync Era", ZKSYNC_RPC_URL, ZKSYNC_PARAMS.to_vec()),
        // Abstract blocks
        (Chain::ZKsync, "Abstract", ABSTRACT_RPC_URL, ABSTRACT_PARAMS.to_vec()),
        // Sophon blocks
        (Chain::ZKsync, "Sophon", SOPHON_RPC_URL, SOPHON_PARAMS.to_vec()),
    ];

    for (chain, chain_name, rpc_url, block_cases) in test_cases {
