        ...
        Ok(
            logs.into_iter()
                .map(|log| {
                    ...
                    let tx_hash = common_data.tx_hash.ok_or_else(|| {
                        anyhow::anyhow!("Missing tx_hash for log primary key")
                    })?;
                    let log_index = common_data.log_index.ok_or_else(|| {
                        anyhow::anyhow!("Missing log_index for log primary key")
                    })?;
